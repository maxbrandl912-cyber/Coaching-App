...
export default function SignupClient({ goTo }) {
  const trainerId = new URLSearchParams(window.location.search).get("trainer");
  ...
  if (!trainerId) {
    return (
      <div className="page">
        <div className="card">
          <div style={{ fontWeight: 600, marginBottom: 8 }}>Einladungslink nötig</div>
          <div style={{ color: "var(--text-dim)", fontSize: 14 }}>
            Für die Klienten-Registrierung brauchst du den Einladungslink von deinem Trainer (mit "?trainer=…" in der URL).
          </div>
        </div>
      </div>
    );
  ...
  const submit = async (e) => {
    ...
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { role: "client", name, trainer_id: trainerId } },
    });
    setLoading(false);
  ...
  return (
    <div className="page">
      <div style={{ fontSize: 26, fontWeight: 700, marginBottom: 4 }}>Klienten-Account erstellen</div>
      <div style={{ color: "var(--text-dim)", fontSize: 14, marginBottom: 24 }}>
        Du wurdest von deinem Trainer eingeladen.
      </div>
